...
namespace LongUI {
    ...
    /// <returns></returns>
    void UIControl::SwapAB(UIControl& a, UIControl& b) noexcept {
        // 必须是1-A-B-2
    ...
    /// <param name="b">The b.</param>
    /// <returns></returns>
    void UIControl::SwapNode(UIControl& a, UIControl& b) noexcept {
        assert(&a != &b && "a == b");
        // A = B
        if (a == b) return;
        // 1 - A - B - 2
        if (a.next == &b) return UIControl::SwapAB(a, b);
        // 1 - B - A - 2
        if (a.prev == &b) return UIControl::SwapAB(b, a);
        // 1 - A - 2        3 - B - 4
        const auto node1 = a.prev;
